...
void for_each_possible_move(position const& p, Fn&& f) {
  ...
  for_each_set_bit(pawns, [&](bitboard const pawn) {
  ...
  for_each_set_bit(knights, [&](bitboard const knight) {
  ...
  for_each_set_bit(bishops, [&](bitboard const bishop) {
    for_each_set_bit(
  ...
  auto rooks = moving_player[ROOK];
  for_each_set_bit(rooks, [&](bitboard const rook) {
    for_each_set_bit(
        get_attack_squares<piece_type::ROOK>(rook, all_pieces) & ~own_pieces,
        [&](bitboard const attack_square) {
          f(move{rook, attack_square});
        });
  });

  auto queens = moving_player[QUEEN];
  for_each_set_bit(queens, [&](bitboard const queen) {
    for_each_set_bit(
  ...
  auto const king = moving_player[KING];
  for (auto const target :
  ...
  auto const can_short_castle = [&]() {
    ...
    for (auto const king_traversal_square :
      ...
      if (get_attack_squares<ROOK>(king_traversal_square, all_pieces) &
              (opposing_player[ROOK] | opposing_player[QUEEN]) ||
          get_attack_squares<BISHOP>(king_traversal_square, all_pieces) &
              (opposing_player[BISHOP] | opposing_player[QUEEN])) {
        return false;
      }
    }
